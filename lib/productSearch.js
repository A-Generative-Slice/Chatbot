...
const searchProducts = async (query, intent = 'general', limit = 5) => {
    ...
    // Store local products for fallback
    ...
    try {
        ...
        if (websiteProducts.length > 0) {
            console.log(`✅ Using ${websiteProducts.length} live products from website`);
            console.log('DEBUG: Sample website product:', JSON.stringify(websiteProducts[0], null, 2));
            ...
            const formattedWebsiteProducts = websiteProducts.map(product => ({
                ...
                categoryKey: 'website',
                source: 'website',
                link: `https://rosechemicals.in/products/${product.id || product._id || product.slug}`,
                image: product.image,
            ...
            // IF WEBSITE PRODUCTS EXIST, USE ONLY THEM (CLEAN DATA)
            allProducts = formattedWebsiteProducts;
        } else {
        ...
        allProducts.forEach(product => {
            const isTargetCategory = (product.source === 'local' && targetCategories.includes(product.categoryKey)) ||
                (product.source === 'website' && targetCategories.some(cat =>
                    product.category?.toLowerCase().includes(cat.split('_')[0]) ||
                    product.name?.toLowerCase().includes(cat.split('_')[0])
                ));
...
const formatProductList = (products, intent, language = 'en-IN') => {
    ...
    products.forEach((product, index) => {
        ...
        const productLink = product.link || `https://rosechemicals.in/products/${product.id || product.slug}`;
        if (productLink) {
