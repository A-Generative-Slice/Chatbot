...
const mapWebsiteProduct = (product) => {
    ...
    return {
        ...
        category: (product.category && typeof product.category === 'object') ? product.category.name : (product.category || 'Website Products'),
        categoryKey: (product.category && typeof product.category === 'object') ? product.category.slug : (product.category?.toLowerCase() || 'website'),
        source: 'website',
...
const searchProducts = async (query, intent = 'general', limit = 5) => {
    ...
    const categoryMappings = {
        ...
        'brush_inquiry': ['toilet-brushes', 'carpet-brushes', 'long-brushes', 'sink-brushes', 'nouful_products', 'china_products', 'global_product_mart'],
        'mop_inquiry': ['mops', 'china_products', 'global_product_mart'],
        'wiper_inquiry': ['wipers', 'global_product_mart'],
        'cleaning_tools_inquiry': ['brooms', 'mops', 'wipers', 'toilet-brushes', 'nouful_products', 'global_product_mart', 'china_products'],
        'floor_cleaner_inquiry': ['floor-cleaners', 'ready_to_use_chemicals', 'diy_kits'],
        'dish_cleaner_inquiry': ['kitchen-cleaners', 'ready_to_use_chemicals', 'diy_kits'],
        'toilet_cleaner_inquiry': ['bathroom-cleaners', 'ready_to_use_chemicals', 'diy_kits'],
    ...
    if (intent && categoryMappings[intent]) {
        ...
        for (const catSlug of targetCategories) {
            try {
                const catResults = await websiteAPI.getProductsByCategory(catSlug);
                const rawCatProducts = catResults.products || [];
    ...
    if (intent && categoryMappings[intent]) {
        ...
        allProducts.forEach(product => {
            const isTargetCategory = (product.source === 'local' && targetCategories.includes(product.categoryKey)) ||
                (product.source === 'website' && targetCategories.some(cat =>
                    product.categoryKey === cat ||
                    product.category?.toLowerCase().includes(cat.split('-')[0]) ||
                    product.name?.toLowerCase().includes(cat.split('-')[0])
                ));
            ...
            if (isTargetCategory) {
                ...
                if (score > 10) {
                    priorityProducts.push({
                        ...product,
                        relevanceScore: score + 50 // Boost for being in matching category
                    });
                }
            }
        });
...
const getProductsByIntent = (intent, allProducts, limit) => {
    ...
    switch (intent) {
        ...
        case 'raw_materials':
            ...
            break;
        case 'broom_inquiry':
            filteredProducts = allProducts.filter(p => p.categoryKey === 'brooms' || p.name.toLowerCase().includes('broom'));
            break;
        case 'mop_inquiry':
            filteredProducts = allProducts.filter(p => p.categoryKey === 'mops' || p.name.toLowerCase().includes('mop'));
            break;
        case 'brush_inquiry':
