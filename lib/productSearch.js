    ...
    try {
        ...
        if (websiteProducts.length > 0) {
            ...
            const formattedWebsiteProducts = websiteProducts.map(product => ({
                ...
                categoryKey: 'website',
                source: 'website',
                link: `${process.env.WEBSITE_URL || 'https://www.rosechemicals.in'}/product/${product.slug}`,
                image: product.image,
...
const formatProductList = (products, intent, language = 'en-IN') => {
    ...
    products.forEach((product, index) => {
        ...
        const productLink = product.link || `${process.env.WEBSITE_URL || 'https://www.rosechemicals.in'}/product/${product.slug || product.id}`;
        if (productLink) {
