...
const findRelevantTutorials = (message) => {
    ...
    let filtered = tutorialsData.filter(t => {
        ...
        // Check for specific product name matches or clear DIY kit matches
        return searchTerms.some(term => title.includes(term));
    });

    // REMOVED LEGACY FALLBACK: Only show tutorials if they actually match the query.
...
const generateResponse = async (message, language = 'en-IN', chatHistory = [], userPhoneNumber = '') => {
    try {
        ...
        // Only show tutorials if we found relevant ones, otherwise keep context clean.
        let tutorialContext = relevantTutorials.length > 0
            ? "\n\nOFFICIAL TUTORIAL VIDEOS (ONLY use these!):\n" + relevantTutorials.map(t => `- ${t.title}: ${t.link}`).join('\n')
            : "";
...
7. ❌ ONLY provide a YouTube link if it is explicitly listed in "OFFICIAL TUTORIAL VIDEOS".
8. ❌ NEVER invent or guess a YouTube link. If no video is listed in context, politely say "I don't have a tutorial video for that yet, but I can guide you!" 🚫🎥
9. ❌ NEVER use generic headers like "Detergent Options at Rose Chemicals" or random emojis like ☁️. 
10. ❌ ALWAYS maintain your persona as "Rose". 
...
2. Provide COMPLETE links accurately. Check every Hex ID before sending.
`;
